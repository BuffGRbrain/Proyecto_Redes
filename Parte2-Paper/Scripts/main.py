...
def print_route_tables(g,L,u,count='inicial'): 
    ...
    plot(grafo, layout=layout,target= f"./paths/Este es el camino más corto en la actualización {count}.png")
    # return df
...
def simulations():
    ...
    for changes in changes_ids:
        for nodes in graph_sizes:
            ...
            loop_update(format_graph(radngraph.gen_graph(nodes)), changes)
...
def main():
    ...
    if bool(config["generate_graph"]):
        t = radngraph.gen_graph(config["nodes"])
        radngraph.graph2csv(t)
    else:
        t = radngraph.import_graph(f"./paths/{config['filename']}.csv")
    ...
    g.es["label"] = g.es["weight"]
    layout = g.layout("kk")
    plot(g, layout=layout,target="./paths/Estado de la red inicial.png")
    loop_update(g, config["changes"])
    print(f"Total Iteraciones: {iteraciones}\nTiempo total: {time.time() - start_time}")
